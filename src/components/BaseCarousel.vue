<template>
  ...
  >
    <ScrollAreaRoot type="auto" class="scroll-carousel__root">
      <ScrollAreaViewport
        ref="slidesRef"
        class="scroll-carousel__viewport"
        :aria-label="description || `${slides.length} items`"
        :tabindex="viewportTabindex"
      >
        <ul
          :id="`${id}-slides`"
          class="scroll-carousel__slides"
          data-ui="scroll-carousel__slides"
        >
          <li
      ...
      <ScrollAreaScrollbar
        class="scroll-carousel__bar scroll-carousel__bar--horizontal"
        orientation="horizontal"
      >
        <ScrollAreaThumb class="scroll-carousel__thumb" />
      </ScrollAreaScrollbar>
    </ScrollAreaRoot>
...
import type { CarouselArrowClickPayload, Carousel, CarouselArrow } from '..';
import { computed, onMounted, onUnmounted, ref } from 'vue';

defineOptions({ name: 'ScrollCarousel' });

const props = withDefaults(defineProps<Carousel>(), {
  id: '',
  slides: () => [],
  description: '',
  dataUi: 'scroll-carousel',
...
const arrows = computed(() => {
  ...
  const isAtRightBoundary =
    currentIndex.value >= props.slides.length - props.slidesToShow;

  return ['left', 'right'].map((direction) => {
    const isEnabled =
      direction === 'left' ? !isAtLeftBoundary : !isAtRightBoundary;

    return {
      direction,
      icon: direction === 'left' ? IconCaretLeft : IconCaretRight,
      attributes: {
        'data-ui': `scroll-carousel__arrow--${direction}`,
        class: `scroll-carousel__arrow scroll-carousel__arrow--${direction} ${
          isEnabled ? '' : 'scroll-carousel__arrow--disabled'
        }`,
...
const onArrowClick = (event: Event, direction: 'left' | 'right') => {
  ...
  currentIndex.value = Math.max(
    0,
    Math.min(proposedIndex, props.slides.length - 1),
  );
...
const debouncedHandleScroll = useDebounceFn(
  (e: Event) => {
    ...
    const positions = props.slides.map((_, index) =>
...
onMounted(() => {
  useEventListener(
    slidesRef.value?.viewportElement,
    'scroll',
...
.scroll-carousel {
  ...
  &__arrow {
    ...
    &--right {
      --extra-slide-width: calc(
        1px * ((var(--slide-width) * var(--slide-extra)))
      );
      --half-gap: calc(var(--slides-gap) / 2 * 1px);
      right: calc(
        var(--extra-slide-width) + var(--half-gap) - var(--button-half)
      );
    }
