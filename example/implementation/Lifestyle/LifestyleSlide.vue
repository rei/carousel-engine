<template>
  ...
  >
    ...
    <template v-if="cta?.text">
      <div v-if="isLifestylePortrait" class="lifestyle-slide__button-container">
        <cdr-button
...
import { CdrButton, CdrLink, CdrImg } from '@rei/cedar';
 ...
 */
const isLifestylePortrait = computed(() =>
  props.slideStyle.startsWith('lifestyle-portrait'),
);

/**
 * Determines if the slide style is a lifestyle square.
 */
const isLifestyleSquare = computed(
  () => props.slideStyle === 'lifestyle-square',
);
...
const buttonSize = computed(() =>
  props.slideStyle === 'lifestyle-portrait-sm'
    ? 'small@xs small@sm small@md'
    : 'medium',
);
